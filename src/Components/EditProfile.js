...
export default function EditProfile() {
  ...
  const handleClick = () => {
    if(fName!=='' || fName!==' '){
      axios.post(`${process.env.REACT_APP_PORT}/user/edit`,{
        fName:fName,
        lName:lName,
        skills:skills
      },{
  ...
  const skillChange = (e) => {
    let check = skills.find(i => {return (i === e.target.value)} )
    if (check===undefined) {
  ...
  return (
    <>
      <div className='outer-container'>
        <p>First Name : <input type="text" ref={First} onChange={(e) => setfName(e.target.value)}></input></p>
        <p>Last Name : <input type="text" ref={Last} onChange={(e) => setlName(e.target.value)}></input></p>
